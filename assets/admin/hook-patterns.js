...
(function () {
    ...
    function init() {
        ...
        document.querySelectorAll('.aegis-hook-pattern-status-toggle').forEach(function (toggle) {
            toggle.addEventListener('change', function () {
                var postId = toggle.getAttribute('data-post-id');
                var enabled = toggle.checked;
                var config = window.aegisHookPatternsList || {};

                if (!postId || !config.nonce) {
                    return;
                }

                jQuery.post(window.ajaxurl, {
                    action: 'aegis_toggle_hook_pattern',
                    nonce: config.nonce,
                    id: postId,
                    enabled: enabled ? 1 : 0
                }).fail(function () {
